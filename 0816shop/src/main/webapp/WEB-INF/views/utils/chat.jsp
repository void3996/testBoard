...
<c:set var="profile" value='<%=session.getAttribute("login")%>' />
	...
	<div class="col-12" style="margin-top: 40px; clear: both;">
		<div class="col-10"
			...
			거래 상품에 대해 문의해보세요. <br>(연락처 문의 또는 직접 알려주는 것은 불가)
		</div>
...
<input type="text" id="nickname" value = "${member.memNickname }" style = "display:none">
 ...
 function send() {
  ...
  if(msg != ""){
	  ...
	  message.message_content = $("#message").val()
  	  message.seller_USER_user_id = '${seller_USER_user_id}'
  	  /* message.USER_user_id = '${profile.user_id}' */
  	  message.gdsnum_gdsnum_id = '${gdsnum_gdsnum_id}'
  	  message.chat_sender = '${user_user_id}'
  }
